...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "ansible-pull -i localhost, -U https://github.com/baludevopsb85/roboshop-ansible roboshop.yml -e role_name=${var.name} -e token=${var.token} env=${var.env} | sudo tee /opt/ansible.log"
    ]
  }
}
